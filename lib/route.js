...
module.exports = function Route(route, options, handler, types) {
    ...
    function normalizeParams(params) {
        for (var name in params) {
            ...
            if (param.source == "query") {
                ...
                throw new Error(
                    "parameter source muste be 'url', 'query' or 'body'"
                );
            }
    ...
    function decodeParams(req, res, next) {
        ...
        for (var key in params) {
            ...
            if (
                ...
                switch (param.source) {
                    case "body":
                        if (param.optional && !(key in body)) break;
                ...
                if (!isValid) {
                    ...
                    if (value !== EMPTY) req.params[key] = value;
    ...
    this.describe = function () {
        var route = {
            route: options.route,
            method: this.method,
            regexp: this.routeRe.toString(),
        };

        if (options.name) route.name = options.name;

        if (options.description) route.description = options.description;
        ...
        if (!Object.keys(route.params).length) delete route.params;
