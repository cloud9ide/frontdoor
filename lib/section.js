...
module.exports = function Section(name, description, types) {
    ...
    this.handle = function(path, req, res, next) {
        ...
        if (arguments.length === 3) {
            ...
            if (!req.parsedUrl) req.parsedUrl = url.parse(req.url, true);
        ...
        function processNext() {
            ...
            if (!handler || i > middleware.length) return next();
    ...
    this.handleError = function(errorHandlers, err, req, res, next) {
        ...
        function processNext(err) {
            ...
            if (!handler || i > errorHandlers.length) return next(err);
    ...
    this.match = function(req, path, method) {
        ...
        for (i = 0; i < methodRoutes.length; i++) {
            ...
            if (route.match(req, path)) return route;
        }
    };
    ...
    this.describe = function() {
        ...
        for (var key in sections) {
            for (i = 0; i < sections[key].length; i++) {
                var section = sections[key][i].describe();
                section.mount = key;
                api.sections.push(section);
            }
        }
        if (!api.sections.length) delete api.sections;

        api.routes = [];
        for (var method in routes) {
            for (i = 0; i < routes[method].length; i++) {
                api.routes.push(routes[method][i].describe());
            }
        }
        if (!api.routes.length) delete api.routes;
