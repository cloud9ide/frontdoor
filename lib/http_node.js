...
function request(url, options, callback) {
    if (!callback) return request(url, {}, options);
    ...
    for (var header in headers) reqOptions.headers[header] = headers[header];
    ...
    var req = proto.request(reqOptions, function (res) {
        ...
        res.on("end", function (chunk) {
            ...
            var err;
            if (status > 299) err = new error.HttpError(responseText, status);
    ...
    req.on("error", function (e) {
        if (debug.enabled) {
            debug("ERROR", reqOptions.method, URL.format(parsedUrl), e);
        }
