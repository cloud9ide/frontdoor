...
exports.Types.prototype.register = function(name, type) {
    type.name = name;
    if (type instanceof RegExp) type = new exports.RegExp(type);
...
exports.Types.prototype.get = function(name) {
    ...
    if (name.check && name.parse) return name;
...
exports.Array.prototype.check = function(value) {
    ...
    for (var i = 0; i < value.length; i++) {
        if (!this.itemType.check(value[i])) return false;
    }
    return true;
};
...
exports.Number.prototype.parse = function(string) {
    ...
    if (isNaN(value)) throw new TypeError("Could not parse string as number");
    return value;
};
