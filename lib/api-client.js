define(function (require, exports, module) {
    ...
    function createClient(endpoint, options, callback) {
        if (!callback) return createClient(endpoint, {}, options);

        var baseUrl =
            options.baseUrl || endpoint.split("/").slice(0, -1).join("/");
        var request =
            options.request ||
            require(
                "./http_node" + "" /* hide this module from the packager */
            );
    ...
    function createSection(description, rest, path) {
        ...
        if (description.sections) {
            description.sections.forEach(function (sectionDesc) {
                ...
                assert(
                    !section[sectionDesc.name],
                    format(
                        "Section with name '%s' already exists",
                        sectionDesc.name
                    )
                );
                section[sectionDesc.name] = createSection(
                    sectionDesc,
                    rest,
                    join(path, sectionDesc.name)
                );
            });
        }
        ...
        if (description.routes) {
            description.routes.forEach(function (route) {
                ...
                assert(
                    !section[name],
                    format("Route with name '%s' already exists", name)
                );
    ...
    function createRoute(route, rest, path) {
        return function request(options, requestOptions, callback) {
            ...
            if (arguments.length == 2)
                return request(arguments[0], {}, arguments[1]);
            ...
            for (key in route.params) {
                ...
                if (!param.optional && !options.hasOwnProperty(key)) {
                    return callback(
                        new TypeError(
                            format("Missing required parameter '%s'", key)
                        )
                    );
                }
            }
            ...
            for (key in options) {
                if (!route.params[key]) {
                    return callback(
                        new TypeError(
                            format(
                                "Argument '%s' is not supported by the API",
                                key
                            )
                        )
                    );
            ...
            for (key in requestOptions.query || {}) {
